...
export function ImageUpload({ imageUrl, onChange }: Props) {
  ...
  async function handleFileChange(event: React.ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      const { data } = supabase.storage.from("flipchart-images").getPublicUrl(path);
  ...
  return (
    <div className="flex flex-col gap-2">
      {imageUrl ? (
        // eslint-disable-next-line @next/next/no-img-element -- designer-authored flip-chart page image, no known dimensions to optimize for
        <img
          src={imageUrl}
          alt=""
