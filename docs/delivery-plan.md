...
**INC-17 — Conversational Chat Guide.** The Chat Guide had been single-turn since INC-4: one string in, one answer out, with `chat_messages` written but never read back and a "did you mean" turn storing the literal string `did_you_mean` with its candidate list discarded. Tapping a suggestion re-POSTed the canonical question text as a brand-new question, so the answer could differ from the option the BHW tapped. This increment adds three things, none of which involve a model. **Red-flag interception** (`content/kb/hhp-ncd/red-flags.json`): authored bilingual phrase rules, matched whole-token before scoring, that force a specific entry regardless of score. They exist because scoring cannot see urgency — verified against the shipped corpus, *"mataas ang presyon niya at sumasakit ang dibdib niya"* scored **0.749 against `m1-hypertension-definition`** (a textbook definition of hypertension), *"high blood pressure with chest pain what do I do"* scored **0.787 against `m3-ph-threshold`** (the 140/90 figure), and *"180 ang presyon niya, ano gagawin ko"* scored **0.709 against `m4-blood-spill`** (how to clean up spilled blood). All three are above the 0.55 answer threshold, so they were returned confidently with no hedge, and `m3-very-high-with-symptoms` — the entry that says to arrange transport now — never won. *"The client is unconscious"* scored 0 against everything and returned `no_answer`. **Clarifiers** (`content/kb/hhp-ncd/clarifiers.json`): when a question names a topic and asks for an action but omits the detail that decides the answer, the system asks one authored bilingual question with 2–4 options instead of guessing — the "deeper question" layer. Options resolve **server-side by index**, so a selection is a selection and never a re-ask, and the client is never told which entry an option maps to. **Follow-up context carry**: a short follow-up ("eh kung bata?") is matched against the previous turn's topic via `chat_sessions.context`, deterministically, with no coreference model. `matchQuestion()` is untouched — `src/lib/chat/conversation.ts` wraps it — so both existing corpora still score identically. Ships behind the `chat_conversation` flag (default `false`). **DoD**: every red-flag fixture routes to its emergency entry, including cases scoring alone answers confidently wrong and one it drops entirely; under-specified questions return a clarifier while already-specific ones on the same topic do not; a selection returns exactly the tapped entry; a clarifier is never logged as a content gap; the MCH and NCD corpora pass unchanged at ≥90%; the flag off restores the previous response byte-for-byte, without a deploy. Scope trimmed for this pass (documented, not silently dropped): rules ship as versioned content files bundled at build time rather than database rows, so changing one needs a deploy and the flag is the no-deploy mitigation — an admin authoring UI is deferred, not rejected; the client model stays `Exchange[]` rather than being rewritten to `Message[]`, because a clarify-then-select pair already renders as a message log and the re-ask defect is fixed by the selection API, not by the state shape; the rate limit stays 20/60s for every request including selections, since a clarify-and-select pair costs 2 of 20 and changing a shared RPC default would affect every caller; 👎 feedback is extended to `did_you_mean` turns but not to `no_answer` (already in the gap queue) or `clarify` (a question, not an answer); only two clarifiers are authored (blood pressure, blood glucose) — more are content work, not code.
...
**INC-18b — the flywheel: AI drafts KB entries from cleared gap questions.** INC-18a built the gate, the budget guard and the adapter and deliberately shipped nothing that spends the quota; this increment is the feature that does, and it is the one the whole arc was for. The loop: a BHW asks something the KB cannot answer → it lands in the gap queue → an admin reads it, **edits it inline to remove anything identifying**, and sends it → the AI returns a bilingual draft entry → the admin reviews and publishes → the next BHW who asks is answered **from the KB, with no AI call at all**. Every call permanently increases coverage, which is what makes "the system relies on the LLM less over time" a measured number rather than an intention — `rpc_dashboard_ai_flywheel` reports drafts created, drafts published, drafts awaiting review and `gap_draft` calls spent, and the E2E test asserts the round trip by re-asking the original question and checking the quota **did not move**. Three pieces carry the increment. **The clearance step** is the DPA guarantee, not a nicety: what leaves is the text the admin typed, never `unmatched_questions.text`, which is why the payload is classified `admin_cleared` rather than `user_generated` — passing the latter throws at the gate, by design. **The review gate** lives in the database, not the form: `rpc_kb_entry_update` refuses to publish a row with `ai_drafted_at` set and `ai_draft_confirmed_at` null, so a direct PostgREST call fails exactly as the UI does; `free-ai-leverage-plan.md`'s risk table says no LLM-generated text reaches a BHW unreviewed, and a UI-only control would make that assertion decorative. **`src/lib/ai/server.ts`** is the composition root, closing INC-18a's loose end where `geminiTransport` was imported by nothing: policy stays in `adapter.ts`, composition and I/O move here, and `eslint.config.mjs` names exactly one exception to the transport-import ban. Also new: the repo's first structured-output path — `ProviderTransport` gains an optional `jsonSchema`, and `parseKbDraft` validates the response as untrusted regardless, **rejecting** a draft with fewer than 4 keywords rather than padding it, on the same reasoning `scripts/lib/kb-content.mjs` applies. Ships behind `ai_gap_draft` (default `false`) under the `ai_external` master switch; both must be on or the route 404s. **DoD**: the route sends `admin_cleared` and provably never the stored gap text; `parseKbDraft` has one case per malformed shape; the publish gate refuses an unreviewed AI draft and allows it after confirmation, verified by executing the migration against a real Postgres; a human-authored entry publishes untouched; confirmation is idempotent and writes one audit event, not one per publish; the existing manual triage path is unchanged (its link locator is the canary — the new control is a `<button>` labelled `"I-draft gamit ang AI"`, sharing no substring with `"Gumawa ng KB Entry Mula Rito"`). Scope trimmed (documented, not dropped): **the E2E flywheel spec is written but not yet executed** — it needs `GEMINI_API_KEY` on the deployment and both flags on, and skips cleanly otherwise, so the round trip is specified and unproven until the key is configured; the draft's category is a placeholder rather than a model choice, because a confidently wrong category is harder for a reviewer to catch than an obviously unset one; no `rpc_kb_entry_delete` — an unwanted draft stays `status='draft'` and invisible to BHWs, and adding delete means answering for the `chat_messages.matched_entry_id` and `unmatched_questions.resolved_entry_id` FKs; no response cache and no provider waterfall; Mode 2 (AI answering a BHW directly) remains INC-19, gated on DOH sign-off.

**Correction (INC-18b follow-up) — the first live switch-on failed, and three things were wrong.** Within an hour of enabling the flags on the pilot, every draft request returned "external AI is unavailable". The cause was not the key: INC-18a had pinned `gemini-2.0-flash` as a **literal in `providers/gemini.ts`**, and Google shut that model down on **2026-06-01**, so every call 404'd. Three defects, each worth naming because each hid the next. **(1) The model was code, not configuration.** A provider retiring a model is routine and inevitable; fixing one should not need a PR, a review and a deploy. The model now comes from `GEMINI_MODEL` (`src/lib/ai/env.ts`), defaulting to Google's stated replacement — the *host* stays a literal because it is a security boundary the lint rule depends on, but the model never was one. **(2) The failure was invisible.** `callProvider` converted the transport error to a value and logged nothing, so the runtime logs were empty; the only evidence the calls had happened at all was `ai_usage.request_count = 4` against `ai.external_call = 0` in the audit table. Degrading silently is required behaviour for the *BHW*; it was never meant to be silent for the *operator*. The adapter now logs the failure, and the transport's message names the HTTP status and the model — built from Google's structured `error.status`/`error.message` only, never a slice of the raw body, because that string reaches Sentry and there is no `beforeSend` scrubber. **(3) One message stood for four causes.** The UI collapsed every 503 into "external AI is unavailable", which reads as "the key is missing" and sent the operator to check the API key that was fine. `aiErrorKey()` now maps `no_api_key` / `flag_disabled` / `over_ceiling` / `timeout` / `provider_error` to five distinct messages, each naming a different next action, with a unit test per branch. The diagnosis that found this took a database query and a deprecation page; with these three changes it would have taken one line of the runtime log.
