...
Remaining later phases (unchanged from the requirements doc, now buildable the same way): Forum → Flip-chart builder → Profiling-system live integration (fallback already ships in INC-2 via manual entry) → offline/PWA.
