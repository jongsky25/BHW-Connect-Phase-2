...
Remaining later phases: Flip-chart builder (INC-14, built independently — see its own PR) → Profiling-system live integration, still blocked on real technical access to the external system (requirements-and-vision.md §7, item 1); INC-2's manual-entry fallback remains the shipped Phase 1 path until that access arrives.
