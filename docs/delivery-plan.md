...
Fixed Phase 1 list: `user.created`, `user.deactivated`, `user.reactivated`, `user.password_reset`, `user.transferred`, `user.consent_given`, `user.data_exported`, `auth.login`, `auth.login_failed`, `auth.lockout`, `kb.entry_created`, `kb.entry_published`, `kb.entry_updated`, `kb.entry_archived`, `gap.resolved`, `gap.dismissed`, `flag.toggled`, `report.exported`. Each event stores a pre-rendered plain-language sentence in both languages (e.g., *"Si Admin Maria ay nagdagdag ng bagong user na si Juan D."*) — the laymanized audit view is just a filtered list of these sentences. Extended in INC-10 with `announcement.created` / `announcement.deleted`, and in INC-12 with `course.created` / `course.status_changed` / `course.deleted` / `assessment.claimed` / `assessment.decided`, following the same pattern.

**A note on audit visibility for non-user subjects (not yet fixed)**: as first documented while building INC-11 (Surveys), `audit_events_admin_read`'s RLS policy only resolves `subject_id` against `users.id`, so any event whose subject is a different entity type — `kb.entry_*`, `flag.toggled`, `gap.*`, `report.exported`, `announcement.*`, `survey.*`, and now INC-12's `course.*`/`assessment.*` — is silently invisible to admins in the audit viewer, regardless of org scope. Still left unfixed here for the same reason: INC-10/11/12 all ship on independent branches, and a real fix needs a single policy aware of every subject-bearing entity type across all of them at once. Tracked as a known gap; needs its own pass once these are merged (or a `subject_org_unit_id` column on `audit_events` to stop re-deriving this per subject type).
