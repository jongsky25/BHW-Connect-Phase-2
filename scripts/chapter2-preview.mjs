...
function figureHtml(a,lang){const alt=esc(a['alt_'+lang]),caption=esc(a['caption_'+lang]),v=(a.videos&&a.videos[lang])||a.video;if(!v)return '<figure><img src="'+a.inline+'" alt="'+alt+'"><figcaption>'+caption+'</figcaption></figure>';return '<figure><video controls'+(a.videos?'':' muted')+' playsinline poster="'+a.inline+'" aria-label="'+alt+'" style="max-width:100%"><source src="'+v.inline+'" type="video/mp4">'+(v.captions?'<track kind="captions" src="'+v.captions.inline+'" srclang="'+lang+'" label="'+(lang==='fil'?'Filipino':'English')+'" default>':'')+'</video><figcaption>'+caption+'</figcaption><details><summary>'+(lang==='fil'?'Mga hakbang bilang teksto':'Steps as text')+'</summary><p>'+alt+'</p></details></figure>'}
function render(){const l=lessons[lessonIndex];document.documentElement.lang=lang;$('module-title').textContent=moduleCode+' · '+moduleTitles[lang];$('lesson').innerHTML=lessons.map((x,i)=>'<option value="'+i+'" '+(i===lessonIndex?'selected':'')+'>'+moduleCode+'.'+(i+1)+' '+esc(x.manifest['title_'+lang])+'</option>').join('');const featured=l.revision.featured_asset_id?l.revision.assets.find(x=>x.id===l.revision.featured_asset_id):null;$('intro').innerHTML='<p class="eyebrow">'+moduleCode+'.'+(lessonIndex+1)+' · '+(position+1)+' / 7</p><h2>'+esc(l.manifest['title_'+lang])+'</h2><p class="muted">'+esc(l.manifest['objectives_'+lang][0])+'</p>'+(featured?'<p class="eyebrow">'+(lang==='fil'?'Panoorin':'Watch')+'</p>'+figureHtml(featured,lang):'');const s=(mode==='read'?l.revision.read_sections:l.revision.slides)[position];let inner='<h3 tabindex="-1" id="section-heading">'+esc(s['heading_'+lang])+'</h3><p class="'+(mode==='slides'?'slide-text':'')+'">'+esc(s[(mode==='read'?'body_':'display_')+lang])+'</p>';for(const id of s.asset_ids){const a=l.revision.assets.find(x=>x.id===id);inner+=figureHtml(a,lang)}if(s.check){const c=s.check;inner+='<p><strong>'+esc(c['prompt_'+lang])+'</strong></p><div class="options">'+c.options.map((o,i)=>'<button data-answer="'+i+'" aria-pressed="'+(answers[answerKey(s)]===i)+'">'+esc(o[lang])+'</button>').join('')+'</div>';if(Object.hasOwn(answers,answerKey(s))){inner+='<div class="feedback" role="status" tabindex="-1"><strong>'+(lang==='fil'?'Paliwanag sa bawat pagpipilian':'Why each option helps or does not help')+'</strong><p>'+esc(c['feedback_'+lang])+'</p><p>'+esc(l.revision.read_sections[position]['takeaway_'+lang])+'</p></div>'}}$('content').innerHTML=inner;$('content').querySelectorAll('[data-answer]').forEach(b=>b.onclick=()=>{answers[answerKey(s)]=Number(b.dataset.answer);render();$('content').querySelector('.feedback')?.focus()});$('bar').style.width=((position+1)/7*100)+'%';document.querySelector('[role=progressbar]').setAttribute('aria-valuenow',position+1);$('previous').disabled=position===0;$('next').disabled=position===6;$('previous').textContent=lang==='fil'?'Bumalik':'Previous';$('next').textContent=lang==='fil'?'Susunod':'Next';$('complete').textContent=lang==='fil'?'Tapusin ang preview':'Complete preview';const attempted=l.revision.read_sections.filter(x=>x.check).every(x=>Object.hasOwn(answers,answerKey(x)));$('complete').disabled=position!==6||!attempted;$('status').textContent=completed.has(l.manifest.lesson_key)?(lang==='fil'?'Natapos sa preview lamang.':'Completed in preview only.'):(lang==='fil'?'Subukan ang dalawang check at puntahan ang huling bahagi.':'Attempt both checks and reach the final section.');}
